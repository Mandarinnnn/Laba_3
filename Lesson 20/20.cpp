...
struct Vertex
{
    ...
    Vertex(Vector3f pos, Vector2f tex)
    {
        m_pos = pos;
        m_tex = tex;
        m_normal = Vector3f(0.0f, 0.0f, 0.0f);//нормаль автоматически инициализируется в 0 через конструктор
    }
};
...
public:
    ...
    bool Init()
    {
        ...
        m_pTexture = new Texture(GL_TEXTURE_2D, "D:/plitk.jpg");
    ...
    virtual void RenderSceneCB()
    {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, m_VBO);// привязываем указатель к названию цели
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), 0);
        glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (const GLvoid*)12);
        glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (const GLvoid*)20);
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_IBO);// привязываем указатель к названию цели
        m_pTexture->Bind(GL_TEXTURE0);
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);//функция для отрисовки
...
private:
    ...
    //В первом цикле только набираем нормали для каждой тройки вершин
    // Во втором цикле сканируем массив вершин напрямую и нормируем нормаль каждой вершины
...
int main(int argc, char** argv)
{
    ...
    if (!GLUTBackendCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, 32, false, "Lesson 20")) {
        return 1;
    }
